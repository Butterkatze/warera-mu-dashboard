...
import './index.css';
import { getWarEraClient } from '../src/api.js'; // Pfad zu deiner api.js anpassen
...
function App() {
  ...
  const fetchArticleAndMu = useCallback(async () => {
    ...
    try {
      ...
      // === ARTIKEL-CACHE PRÜFEN ===
      const articleCacheKey = `article_cache_${ARTICLE_ID}`;
      const cachedArticle = localStorage.getItem(articleCacheKey);
      const articleCacheTime = localStorage.getItem(`${articleCacheKey}_time`);
      const ARTICLE_CACHE_DURATION = 2 * 60 * 60 * 1000; // 2 Stunden
  
      if (cachedArticle && articleCacheTime && (Date.now() - Number(articleCacheTime) < ARTICLE_CACHE_DURATION)) {
        text = cachedArticle;
      } else {
        // Wenn kein Cache da ist (oder gelöscht wurde), API fragen
        const articleData = await client.article.getArticleById({ articleId: ARTICLE_ID });
        text = articleData.content || "";
        
        localStorage.setItem(articleCacheKey, text);
        localStorage.setItem(`${articleCacheKey}_time`, Date.now().toString());
      }
  ...
  const handleManualRefresh = async () => {
    setLoading(true); // Schaltet sofort den Lade-Schnitt ein
  
    // 1. Schau in den Speicher und sammle ALLE Keys, die mit mu_ oder user_ beginnen
    const keysToRemove = [];
    for (let i = 0; i < localStorage.length; i++) {
      const key = localStorage.key(i);
      if (key && (key.startsWith('mu_cache_') || key.startsWith('user_cache_'))) {
        keysToRemove.push(key);
      }
    }
  
    // 2. Lösche NUR diese Keys aus dem localStorage
    keysToRemove.forEach(key => localStorage.removeItem(key));
    console.log(`Manuelles Update: ${keysToRemove.length} Caches bereinigt.`);
  
    // 3. Erst JETZT rufen wir deine ganz normale Ladefunktion auf.
    // Da der Speicher jetzt leer ist, wird sie automatisch gezwungen, die echten APIs anzufragen!
    await fetchArticleAndMu();
  };
  ...
  return (
    <div className="container">
      ...
      <header>
        ...
        <button className="update-btn" onClick={handleManualRefresh} disabled={loading}>
